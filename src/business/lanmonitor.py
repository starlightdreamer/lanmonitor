#!/usr/bin/env python3
#
# LanMonitor is a Windows command-line program intended to alert the user when a new connection has been added to
# their home wifi. A list of LAN devices is provided, with the ability to save custom device names, or use default
# manufacturer names from an API call to a third-party MAC address manufacturer lookup service.
#
# Process flow:
# parse arguments
...
from src.constants.patterns import IP_ADDRESS_PATTERN, MAC_ADDRESS_PATTERN
from src.ui.informer import inform

# for command-line argument determining whether or not to prompt the user for custom naming of devices.
ask_name_change = False

# for command-line argument to contain list of ints, of device ID #'s to be alerted with Windows pop-up.
watched_devices = []
...
def connection_notice(device):
    ...
    for d in watched_devices:
        if d == device['id']:
            inform('Watched device named ' + device['name'] + ' connected.',
                   'Device #' + str(device['id']) + " named " + device['name'] + " connected.")
...
def disconnection_notice(device):
    ...
    for d in watched_devices:
        if d == device['id']:
            inform('Watched device named ' + device['name'] + ' disconnected.',
                   'Device #' + str(device['id']) + " named " + device['name'] + " disconnected.")
...
def run():
    ...
    program_description = "Lan Monitor can alert the user when a new connection has been added to their home wifi.\n" + \
                          "Devices on the LAN have their names stored in SQLite file named clients.db in program " \
						  "folder.\n "
    ...
    argparser.add_argument('-m', '--monitor', action='store_true',
                           help='provide id #\'s of devices to track, based on their stored ID#')
